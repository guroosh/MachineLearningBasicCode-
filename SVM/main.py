...
def computeCost(X, y, theta, lambd):
    ...
    m = len(X)
    for i in range(m):
        cost += max(0, 1 - y[i] * theta.dot(X[i])) + lambd * theta.dot(theta)
    return cost / m
...
def computeGradient(X, y, theta, lambd):
    ...
    sum1 = np.zeros(n)
    for i in range(m):
        sum1 += y[i] * X[i] * 1 if (y[i] * theta.dot(X[i]) <= 1) else 0
    grad = 2 * lambd * theta - (sum1 / m)
    return grad
...
def addQuadraticFeature(X):
    ...
    ##### insert your code here #####
    new_X = np.zeros((np.shape(X)[0], np.shape(X)[1] + 1))
    new_X[:, :-1] = X
    new_X[:, -1] = X[:, 0] * X[:, 0]
    return new_X
...
def main():
    ...
    score = computeScore(Xt, y, preds)
    print('accuracy from svm = {0:.2f}%'.format(score / len(y) * 100))
